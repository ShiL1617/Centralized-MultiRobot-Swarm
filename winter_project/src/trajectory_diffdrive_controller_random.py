...
TRAJECTORY_SAMPLE_RATE = 0.01 # seconds
...
class DiffDriveVelocityController( object ):
    ...
    def trajectorygen_callback(self):
        ...
        for i in range(N):
            ...
            # random path generator from superellipse inspired paths

            r_vector = [0.5, 1.0, 2.0]
            # random_path = np.random.choice(r_vector,replace=False)

            if normalize_angle_positive(t) >= 0. and normalize_angle_positive(t) <= (pi/2.):
